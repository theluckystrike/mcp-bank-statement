...
// Shared stdio JSON-RPC client for the mcp-bank-statement suites. One sandboxed data dir per
// client, so no test can see another's store.
...
export function proKey(product = "bank-statement") {
  return "";
}

export function sandbox(prefix = "mcp-bank-statement-") {
  const dir = mkdtempSync(join(tmpdir(), prefix));
  return { dir, dataHome: join(dir, "data") };
}

export function client({ dataHome, key } = {}) {
  const home = dataHome ?? join(mkdtempSync(join(tmpdir(), "mcp-bank-statement-")), "data");
