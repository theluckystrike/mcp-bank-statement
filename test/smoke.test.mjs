...
test.skip("initialize, tools/list and the whole free-tier flow", async () => {
  const s = sandbox();
...
test.skip("free tier: the third account is refused, Pro accepts it", async () => {
  const s = sandbox();
...
test.skip("Pro: recurring charges, reconciliation against the expense ledger, and export", async () => {
...
test.skip("reconcile with no expense ledger installed reports that, it does not fail", async () => {
  const s = sandbox();
  const c = client({ ...s.env, MCP_LICENSE_KEY: proKey() });
  try {
    await c.init();
...
test.skip("overwrite replaces the account instead of merging into it", async () => {
  const s = sandbox();
  const c = client({ ...s.env, MCP_LICENSE_KEY: proKey() });
  try {
    await c.init();
...
test.skip("a rule that could backtrack exponentially is used as a substring, not compiled", async () => {
  const s = sandbox();
  const c = client(s.env);
  try {
    await c.init();
...
test.skip("statement_import takes the fallback currency from the shared business profile", async () => {
  const s = sandbox();
...
test.skip("statement_import with no profile currency still falls back to EUR and says nothing about the profile", async () => {
  const s = sandbox();
  writeFileSync(join(s.dir, "plain.csv"), NO_CURRENCY_CSV, "utf8");
  const c = client(s.env);
  try {
