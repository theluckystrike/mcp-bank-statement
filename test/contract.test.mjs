...
// Contract suite for bank-statement. Mechanical assertions only: the invariants of
// servers/bank-statement/SPEC.md that a test can check without judgement.
...
test("corrupt store: the garbage is quarantined byte-for-byte and nothing is overwritten", async (t) => {
  ...
  t.after(() => { c.close(); cleanup(box.dir); });
  await c.init();
  const r = await c.call("transactions_list", { from: "2026-01-01", to: "2026-01-31" });
...
test("cap: statement_export is Pro and writes no file at all on free", async (t) => {
  const box = sandbox();
  const c = client({ dataHome: box.dataHome });
  t.after(() => { c.close(); cleanup(box.dir); });
  await c.init();

  const out = join(box.dir, "export.csv");
  const r = await c.call("statement_export", { from: "2026-01-01", to: "2026-01-31", format: "csv", path: out });
